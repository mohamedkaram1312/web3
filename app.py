...
if st.button("Run Prediction"):
    ...
    def compute_indicators(data):
        ...
        data['%K'] = 100 * ((data['Close'] - data['L14']) / (data['H14'] - data['L14'])).fillna(50)
    ...
    if data.empty:
        ...
        st.write(f"Predicted price for {ticker} next month: ${predicted_price:.2f}")
        
        # Plotting
        fig, ax = plt.subplots(figsize=(14, 7))
        ax.plot(data['Close'], label='Historical Prices', color='blue')
        ax.scatter(data.index[-1] + pd.Timedelta(days=30), predicted_price, color='red', s=100,
