...
def analyze_stock(ticker, start_date, end_date):
    ...
    data['OBV'] = calculate_obv(data)  # Added OBV calculation

    # Drop rows with NaN values (caused by rolling windows)
    data = data.dropna()

    # Prepare data for scaling (Close, SMA_50, SMA_200, EMA_50, RSI_3, RSI_5, RSI_10, RSI_14, RSI_20, RSI_25, OBV)
    scaler = MinMaxScaler()
    scaled_data = scaler.fit_transform(data[['Close', 'SMA_50', 'SMA_200', 'EMA_50', 
                                             'RSI_3', 'RSI_5', 'RSI_10', 'RSI_14', 'RSI_20', 'RSI_25', 'OBV']])  # Included OBV
