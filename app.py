...
if st.button("Analyze"):
    ...
    if percentage_increase is not None:
        ...
        plt.plot([last_price, predicted_price], marker='o')
        plt.title(f"Last vs Predicted Price for {ticker}")
        plt.xticks([0, 1], ['Last Price', 'Predicted Price'])
        plt.ylabel("Price")
        st.pyplot(plt)
    else:
        st.warning(f"Analysis failed for {ticker}.")
